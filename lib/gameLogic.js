...
const VALID_QUESTION_SETS = new Set(['set1', 'set2', 'set3', 'set4', 'set5', 'test']);
...
function applyVotingResults(gameState) {
  ...
  const changes = {};
  const liarCaught = accusedName === gameState.liarName && maxVotes >= 3;
  const innocentCaught = accusedName !== null && accusedName !== gameState.liarName && accusedName !== 'ABSTAIN' && maxVotes >= 3;
  const isRound10 = gameState.currentRound === 10;
  let recovered = 0;
  const liarMultiplier = gameState.powerupsThisRound[gameState.liarName] ? 2 : 1;
  ...
  if (liarCaught) {
    if (gameState.players[gameState.liarName]) {
      ...
      const penalty = Math.floor((gameState.players[gameState.liarName].pointsSinceLastVote || 0) / 2);
    ...
    Object.entries(gameState.votes).forEach(([voterName, votedFor]) => {
      if (votedFor === gameState.liarName && voterName !== gameState.liarName && gameState.players[voterName]) {
        const mult = gameState.powerupsThisRound[voterName] ? 2 : 1;
