...
function getAnswerVariants(answer) {
  return [answer.text, ...(Array.isArray(answer.aliases) ? answer.aliases : [])]
    .map(normalize)
    .filter(Boolean);
}
...
function matchAnswer(input, answers, revealedIdxs = []) {
  ...
  for (let i = 0; i < answers.length; i++) {
    ...
    if (getAnswerVariants(answers[i]).includes(normInput)) return i;
  }
  ...
  for (let i = 0; i < answers.length; i++) {
    if (revealed.has(i)) continue;
    const variants = getAnswerVariants(answers[i]);

    for (const normAnswer of variants) {
      ...
      if (normInput.length >= 3) {
        ...
        if (dist <= threshold) {
          ...
          fuzzyCandidates.push({ index: i, dist, answerLength: normAnswer.length });
        }
      }
    }
  }

  fuzzyCandidates.sort((a, b) => a.dist - b.dist || a.answerLength - b.answerLength);
  if (fuzzyCandidates.length === 0) return -1;

  const best = fuzzyCandidates[0];
  const sameBest = fuzzyCandidates.filter(c => c.dist === best.dist && c.index !== best.index);
  if (sameBest.length > 0) return -1;
...
function getValidVoteTargets({ phase, playerName, votedName, players, top2 = [], protectedVoteTargets = [] }) {
  if (phase === 'finalVoting') {
    if (top2.includes(playerName)) return [];
    return top2.filter(name => players[name]);
  }
  if (phase === 'voting') {
    const protectedSet = new Set(protectedVoteTargets);
    const targets = Object.keys(players).filter(name => name !== playerName && !protectedSet.has(name));
...
function validateVote({ phase, playerName, votedName, players, top2 = [], protectedVoteTargets = [] }) {
  ...
  const targets = getValidVoteTargets({ phase, playerName, votedName, players, top2, protectedVoteTargets });
  ...
  if (liarCaught) {
    ...
    Object.entries(gameState.votes).forEach(([voterName, votedFor]) => {
      if (votedFor === accusedName && gameState.players[voterName]) {
        const mult = gameState.powerupsThisRound[voterName] ? GAME_CONFIG.powerupMultiplier : 1;
        const penalty = GAME_CONFIG.wrongAccusationPenalty * mult;
        if (penalty > 0) {
          gameState.players[voterName].score = Math.max(0, gameState.players[voterName].score - penalty);
          changes[voterName] = -penalty;
        } else {
          changes[voterName] = 0;
        }
      }
    });
    gameState.hiddenLiarPoints += GAME_CONFIG.hiddenLiarReward;
  }
